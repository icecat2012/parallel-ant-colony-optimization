...
// for sse 
__m128 decline = _mm_set1_ps(0.99);
...
void* run_ants(void* data)
{
	...
	while(true){
	    ...
	    if(thread_id == 0)
		{
	    	__m128 tmp;
	  	    __m128 result;
	    	for(int i = 0; i < tpoints; i+=4){
	    		tmp = _mm_load_ps(&blue_horm[i]);
	    		result = _mm_mul_ps(decline, tmp);
	    		_mm_store_ps(&blue_horm[i], result);
			}
	    	for(int i = 0; i < tpoints; i+=4){
	    		tmp = _mm_load_ps(&red_horm[i]);
	    		result = _mm_mul_ps(decline, tmp);
	    		_mm_store_ps(&red_horm[i], result);
			}
		}
